...
export default function LiderAnunturiPage() {
  ...
  const submit = async () => {
    if (!form.title.trim() || !form.content.trim()) {
      setMsg('⚠️ Completează titlul și conținutul!')
      return
    }
    setSub(true)
    const r = await fetch('/api/anunturi', {
  ...
  const remove = async (id: string) => {
    ...
    await fetch(`/api/anunturi/${id}`, { method: 'DELETE' })
    await load()
  }
      ...
      <div className="grove-card">
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <label htmlFor="important" className="text-sm text-zinc-400 cursor-pointer">
              ⚠️ Marchează ca Important
            </label>
          </div>
      ...
      <div className="grove-card">
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {anunturi.map(a => (
              <div key={a.id} className="flex items-start justify-between gap-3 p-3 rounded-xl bg-dark-hover border border-dark-border">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-zinc-600 mt-0.5">
                    {format(new Date(a.createdAt), 'dd MMM yyyy HH:mm', { locale: ro })}
                  </div>
                </div>
