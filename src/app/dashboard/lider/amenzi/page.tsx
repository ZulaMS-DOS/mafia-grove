...
import { AlertTriangle, Plus, Trash2, RefreshCw, Shield } from 'lucide-react'
...
export default function LiderAmenziPage() {
  ...
  const resetAmenda = () => { setUserId(''); setMaterial(''); setBucati(''); setTermen(''); setFwLevel('0') }
  ...
  return (
    <div className="space-y-5 animate-slide-up">
      ...
      {/* Tab-uri — Tester vede doar Amenzi */}
        ...
        <div className="space-y-4">
          <div className="grove-card space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div className="md:col-span-2">
                <label className="grove-label">Nivel FW</label>
          ...
          <div className="grove-card p-0 overflow-hidden">
            <div className="px-5 py-3 border-b border-dark-border">
              <h2 className="text-sm font-semibold text-zinc-500 uppercase tracking-widest">{fwuri.length} Faction Warn-uri</h2>
            </div>
            {loading ? (
              <div className="text-center py-8 text-zinc-600">Se încarcă...</div>
            ) : fwuri.length === 0 ? (
              ...
              <div className="divide-y divide-dark-border/50">
                {fwuri.map(f => <FineCard key={f.id} f={f} />)}
              </div>
            )}
          </div>
        </div>
